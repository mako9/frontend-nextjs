...
export async function getAllCommunities(context?): Promise<Page<Community> | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community`;
  const result = await request<Page<Community>>(url, context);
  return result.data;
}

export async function getOwnedCommunities(session?): Promise<Page<Community> | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community/owned`;
  const result = await request<Page<Community>>(url, session);
  return result.data;
}

export async function getMyCommunities(session?): Promise<Page<Community> | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community/my`;
  const result = await request<Page<Community>>(url, session);
  return result.data;
}

export async function getCommunity(uuid, session?): Promise<Community | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community/${uuid}`;
  const result = await request<Community>(url, session);
  return result.data;
}

export async function createCommunity(community: Community, session?): Promise<Community | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community`;
  const result = await request<Community>(url, session, HttpMethod.Post, community);
  return result.data;
}

export async function editCommunity(community: Community, session?): Promise<Community | null> {
...
export async function deleteCommunity(uuid, session?): Promise<Community | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community/${uuid}`;
  const result = await request<Community>(url, session, HttpMethod.Delete);
  return result.data;
}

export async function getItemsOfCommunity(uuid, session?): Promise<Page<Item> | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community/${uuid}/item`;
  const result = await request<Page<Item>>(url, session);
  return result.data;
}

export async function getMemberOfCommunity(uuid, session?): Promise<Page<User> | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community/${uuid}/member`;
  const result = await request<Page<User>>(url, session);
  return result.data;
}

export async function getReguestingMemberOfCommunity(uuid, session?): Promise<Page<User> | null> {
  const url = `${process.env.BACKEND_BASE_URL}/user/community/${uuid}/requesting-member`;
  const result = await request<Page<User>>(url, session);
  return result.data;
}

export async function approveMemberRequests(uuid, userUuids, session?): Promise<Page<User> | null> {
...
export async function declineMemberRequests(uuid, userUuids, session?): Promise<Page<User> | null> {
