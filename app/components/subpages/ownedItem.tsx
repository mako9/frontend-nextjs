...
export default function ownedItem(
    session,
    itemData: Item,
    itemImageData,
    myCommunities,
    ...
    const initialCommunity = myCommunities.find(
        (e) => e.uuid === itemData.communityUuid
    )
    const [community, setCommunity] = useState<DropdownOption | null>(
        initialCommunity
            ? { id: initialCommunity.uuid, label: initialCommunity.name }
            : null
    )
    const initialCategories = itemData.categories.map((category) =>
    ...
    const deleteItemImageWithLoading = clientSideRequest(
        deleteItemImage,
        state,
        setState
    )
    const uploadImageWithLoading = clientSideRequest(
        uploadItemImage,
        state,
        setState
    )
    ...
    const onHandleItemImageDelete = async () => {
        await deleteItemImageWithLoading(
            itemImageData[currentImageIndex],
            session
        )
