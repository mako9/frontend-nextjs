...
export default function foreignCommunity(session, communityData, t) {
    const Map = dynamic(() => import('../map'), {
        loading: () => (
            <div className={`${styles.map} ${styles.map_default_background}`}>
                <Spinner />
            </div>
        ),
        ssr: false,
    ...
    return (
        <div>
            ...
            <div className={keyValueStyles.key_value_horizontal_container}>
                ...
                <KeyValue
                    name={t('houseNumber')}
                    value={communityData.houseNumber}
                />
            </div>
            <div className={keyValueStyles.key_value_horizontal_container}>
                <KeyValue
                    name={t('postalCode')}
                    value={communityData.postalCode}
                />
            ...
            <br />
            <Button
                title={t('community.requestMembership')}
                onClick={() => joinCommunity(communityData.uuid, session)}
                isDisabled={!communityData.canBeJoined}
            />
        </div>
    )
}
