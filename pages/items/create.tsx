...
export default function createItemPage({ myCommunities }) {
    ...
    const [community, setCommunity] = useState(myCommunities.content.length > 0 ? ({ id:  myCommunities.content[0].uuid, label:  myCommunities.content[0].name }) : null);
    ...
    const onHandleImageUpload = async (uploadImageFile: File) => {
      const newImageUrls = imageUrls.concat([URL.createObjectURL(uploadImageFile)]);
    ...
    return (
      <div>
        ...
        <article>
          <h1 className={utilStyles.headingXl}>{t('myArea.createItem')}</h1>
        </article>
        <br/>
        <h3>{t('name')}</h3>
        ...
        <br/>
            ...
            <Dropdown
                options={myCommunities.content.map(e => ({ id: e.uuid, label: e.name }))}
